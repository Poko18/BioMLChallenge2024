...
parser.add_argument('--num_seqs', type=int, default=80, help='number of sequences to generate (default: 80)')
...
def relax_structure(pdb_file: str, relaxed_pdb_path: str) -> None:
    """Relax a PDB structure and save it. Acknowledgment: Martin Pacesa, RosettaCommons slack."""
    ...
    try:
        ...
        # Generate MoveMapx
        movemap = MoveMap()
        movemap.set_chi(True) #sidechain movement
        movemap.set_bb(True) # backbone movement
        movemap.set_jump(False) # Disable whole chain movement
        # Relax
        fastrelax = FastRelax()
        fastrelax.set_scorefxn(get_fa_scorefxn())
        fastrelax.set_movemap(movemap)
        fastrelax.max_iter(200) # reduced
        ...
        logger.info(f"Structure relaxed and saved to {relaxed_pdb_path}")
    except Exception as e:
        logger.error(f"Error during structure relaxation: {e}", exc_info=True)

def select_surface_binder_residues(pdb: str, binder_chains: List[str]) -> np.ndarray:
    """Select surface binder residues from the PDB file for the specified binder chains."""
...
polar_bias_value = 0.6
binder_sel = select_surface_binder_residues(pdb, binder_chains)
logger.info(f"Surface binder residues: {[i+1 for i, x in enumerate(binder_sel) if x]}")
logger.info(f"Adding bias ({polar_bias_value}) to surface residues: {surface_aa}")
...
logger.info(f"Sampling {num_seqs} sequences with MPNN model")
...
logger.info("Running AF2 on MPNN sequences")
...
for n in range(num_seqs):
    ...
    logger.info(f"Designing sequence {n+1}/{num_seqs} with sequence {seq}")
    af_model.predict(seq=seq, num_recycles=num_recycles, num_models=1, verbose=False)
    logger.debug(f"log: {af_model.aux['log']}")
    ...
    save_pdb = not args.save_best_only or (
